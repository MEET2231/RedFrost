...
public class PlayerMovementScript : MonoBehaviour {
	...
	public float defaultTempo = 0.3f;
	...
	*/
	void FixedUpdate(){
		...
		Timekeeper.tempo = currentSpeed / maxSpeed + defaultTempo;
	}
	/*
	...
	void PlayerMovementLogic(){
		...
		if (grounded){
			rb.velocity = Vector3.SmoothDamp(rb.velocity,
				...
				ref slowdownV,
				deaccelerationSpeed)*speedMultiplier;
		}
	...
	*/
	void Update(){
		...
		if (Input.GetKeyDown(KeyCode.H))
		{
			...
			slowdownV = Vector3.zero;
		}
		
		Debug.Log(Timekeeper.tempo);
